...
inline uint32_t la_parity_read_u32(const std::string& path, const std::string& key) {
...
// Read a uint32 KV entry (alias of la_parity_read_u32, matching the box-parse gate).
inline uint32_t load_kv_u32(const std::string& path, const std::string& key) {
    return la_parity_read_u32(path, key);
}

// Load a string-array KV entry from a gguf (e.g. tokenizer.tokens). Returns false if
// the key is absent or is not a string array.
inline bool load_kv_str_array(const std::string& path, const std::string& key,
    ...
    if (id >= 0 && gguf_get_kv_type(g, id) == GGUF_TYPE_ARRAY &&
        ...
        size_t nn = gguf_get_arr_n(g, id);
        out.resize(nn);
        for (size_t i = 0; i < nn; ++i) out[i] = gguf_get_arr_str(g, id, i);
        ok = true;
    }
    gguf_free(g);
    return ok;
