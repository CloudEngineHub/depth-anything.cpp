...
"""
import os, sys, types, subprocess, numpy as np
...
def main():
    ...
    img_path = sys.argv[1] if len(sys.argv) > 1 and os.path.exists(sys.argv[1]) else None
    if img_path:
    ...
    with torch.no_grad():
        outs, _ = bb.get_intermediate_layers(
            x, n=[5, 7, 9, 11], export_feat_layers=[], ref_view_strategy="saddle_balanced")
    ...
    model = os.path.join(ROOT, "models", "depth-anything-base-f32.gguf")
