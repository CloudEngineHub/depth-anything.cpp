// M7 Task 1 — GGUF quantization (DA3 matmul weights only).
//
// Mirrors the dequant -> ggml_quantize_chunk -> owned-buffer ->
// gguf_add_tensor -> gguf_write_to_file mechanism.
//
// CRITICAL: only the 2D Linear/matmul weights consumed via ggml_mul_mat are
// quantized (vit backbone attn/mlp + cam decoder fc layers). The conv kernels
// (head.*, vit.patch_embed), norms, biases, vit.pos_embed, vit.cls_token and
...
bool quantize_gguf(const std::string& in_gguf, const std::string& out_gguf, const std::string& type) {
    ...
    for (int64_t ti = 0; ti < n_tensors && !failed; ++ti) {
        ...
        if (should_quantize(name) && ggml_n_dims(src) == 2) {
            if (src->ne[0] % 256 == 0) {
                used = target;       // K-quants need 256-divisible rows; DA3 dims -> OK
                quant = true;
            } else if (is_kquant && src->ne[0] % 32 == 0) {
                used = GGML_TYPE_Q8_0; // safety fallback (won't trigger for DA3 dims)
